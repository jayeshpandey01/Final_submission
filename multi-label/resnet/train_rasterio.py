...
from dataloader_rasterio import GenDEBRIS_ML, bands_mean, bands_std, RandomRotationTransform , pos_weight, gen_weights
...
def main(options):
    ...
    if options['mode']=='train':
        ...
        # Handle prefetch_factor and persistent_workers for single-threaded mode
        dataloader_kwargs = {
            'batch_size': options['batch'],
            'num_workers': options['num_workers'],
            'pin_memory': options['pin_memory'],
            'worker_init_fn': seed_worker if options['num_workers'] > 0 else None
        }
